...
// content is the int to display
// will be converted to char, and placed at (x, y)
...
//    unsigned char parsed_digit_array[5] = {0,0,0,0,0}
    unsigned char display_char[7] = {' ', ' ', ' ', ' ', ' '};
    unsigned char i = 0;
    unsigned int remainder = content;
    unsigned char parsed_digit = 0;

    for (i=0; i<5; i++){
//        parsed_digit_array[4-i] = remainder % 10;
        parsed_digit = remainder % 10;
        display_char[5-i] = parsed_digit + '0';
        remainder = (unsigned int) remainder / 10;
    }

    Graphics_drawStringCentered(&g_sContext, display_char, 7, 48, 48, TRANSPARENT_TEXT);
    Graphics_flushBuffer(&g_sContext);
...
void mainLoopLab1(){
    ...
    while (true){
        ...
        switch (state){
            ...
            // make a matrix of random number as alian
            // number of row = level + 1
            case (MAKE_ALIAN):
                if (loop_counter == 2){
                    Graphics_clearDisplay(&g_sContext);
